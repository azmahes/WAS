...
define websphere::profile::appserver (
  ...
  if ! $options {
    $_options = "-create -profileName ${profile_name} -profilePath ${profile_base}/${profile_name} -templatePath ${_template_path} -nodeName ${node_name} -hostName ${dmgr_host} -federateLater true -cellName ${cell}"
  } else {
    $_options = $options
  }
